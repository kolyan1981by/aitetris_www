...
class BBLogic
  ...
  def self.bonus(l,r,rr, curr_piece, orient)
    ...
    when ['L',0];
      bb+=1 if r==w+1 && fit_tmpl(l+1,'0 0 0',rr) 
      bb+=1 if l==0 && fit_tmpl(l+1,'0 0',rr)
  ...
  def self.anlz_S(rr)
    ...
    for i in 1..rr.size-1
      ...
      t = "0 d1";       res<< [[i,r0-1],1,r0+2,t, add_gap(r0-r1-1,5)+bonus(i-1,i+2,rr,'S',1)]    if fit_tmpl(i,t, rr) #gap
